...
contract Ownable {
    ...
    event replaceOwner(address newOwner);

    //  2) create an internal constructor that sets the _owner var to the creater of the contract
    constructor() public {
        _owner = msg.sender;
        emit replaceOwner(msg.sender);
    }
    //  3) create an 'onlyOwner' modifier that throws if called by any account other than the owner.
    modifier onlyOwner() {
        require(msg.sender == _owner, "You are not the authorized owner");
        _;
    }
...
//  2) create a public setter using the inherited onlyOwner modifier
    function setterPaused() public onlyOwner {
        _paused = true;
        emit pauseContract(msg.sender);
    }

    function setterUnpaused() public onlyOwner {
        _paused = false;
        emit unpauseContract(msg.sender);
    }
...
//  4) create 'whenNotPaused' & 'paused' modifier that throws in the appropriate situation
    modifier whenNotPaused() {
        require(_paused == false, "Do not pause this contract.");
        _;
    }

    modifier paused() {
        require(_paused == true, "You need to pause this contract.");
        _;
    }

//  5) create a Paused & Unpaused event that emits the address that triggered the event
    event pauseContract(address owner);
    event unpauseContract(address owner);
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        address _notOwner = ownerOf(tokenId);
        require(to != _notOwner, "Different address required.");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(this.ownerOf(tokenId) == msg.sender, "You do not own this token");
        // TODO add 'to' address to token approvals
        _tokenApprovals[tokenId] = to;
        // TODO emit Approval Event
        emit Approval(_notOwner, to, tokenId);
    }
